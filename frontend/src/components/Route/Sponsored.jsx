import React from "react";
import styles from "../../styles/styles";

const Sponsored = () => {
  const brands = [
    { name: "Sony", logo: "https://logos-world.net/wp-content/uploads/2020/04/Sony-Logo.png" },
    { name: "Dell", logo: "https://logos-world.net/wp-content/uploads/2020/08/Dell-Logo-1989-2016.png" },
    { name: "LG", logo: "https://upload.wikimedia.org/wikipedia/commons/thumb/8/8d/LG_logo_%282014%29.svg/600px-LG_logo_%282014%29.svg.png" },
    { name: "Brand 4", logo: "https://www.shutterstock.com/image-vector/galati-romania-april-29-2023-600nw-2295394661.jpg" },
    { name: "Brand 5", logo: "data:image/png;base64," },
  ];

  return (
    <div className={`${styles.section} hidden sm:block bg-gradient-to-br from-gray-50 to-white py-12 px-5 mb-12 rounded-xl shadow-sm border border-gray-100`}>
      <div className="mb-6">
        <h3 className="text-2xl font-bold text-center text-gray-800 mb-2">Đối Tác & Thương Hiệu</h3>
        <p className="text-center text-gray-600">Những thương hiệu uy tín hàng đầu</p>
      </div>
      <div className="flex flex-wrap justify-center items-center gap-8 md:gap-12 lg:gap-16">
        {brands.map((brand, index) => (
          <div
            key={index}
            className="flex items-center justify-center p-4 bg-white rounded-lg shadow-sm hover:shadow-md transition-all duration-300 hover:scale-105 border border-gray-200"
            style={{ width: "150px", height: "100px" }}
          >
            <img
              src={brand.logo}
              alt={brand.name}
              className="max-w-full max-h-full object-contain filter grayscale hover:grayscale-0 transition-all duration-300"
              onError={(e) => {
                e.target.src = "https://via.placeholder.com/150x100?text=Brand";
              }}
            />
          </div>
        ))}
      </div>
    </div>
  );
};

export default Sponsored;
